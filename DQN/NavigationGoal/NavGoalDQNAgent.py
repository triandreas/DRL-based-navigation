...
class NavGoalDQNAgent:
    ...
    def build_model(self):
        # The method that builds the neural net for the Deep-Q learning model                
        model = Sequential([
            Input(shape = 7),   # 7 --> the dimension of the state array (7,)
            Dense(64, activation = "tanh"), # prev 64
            Dense(128, activation = "tanh"), # prev 128
            Dense(len(self.action_space), activation = None)  # len(self.action_space) --> the output num: number of possible actions i.e. 3 (forward, rotate_left, rotate_right)            
        ])
